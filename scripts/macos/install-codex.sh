#!/usr/bin/env sh
# Connect Codex to the Verbex MCP server by adding a 'verbex' entry to ~/.codex/config.json.
# The Verbex MCP server requires no authentication.
# Usage: sh install-codex.sh
# Override with VERBEX_MCP_URL / VERBEX_CODEX_CONFIG.
set -e
...
servers["verbex"] = {"type": "http", "url": url}
