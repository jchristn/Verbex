@echo off
REM Connect Cursor to the Verbex MCP server by adding a 'verbex' entry to %USERPROFILE%\.cursor\mcp.json.
REM The Verbex MCP server requires no authentication.
REM Usage: install-cursor.bat
REM Override with VERBEX_MCP_URL / VERBEX_CURSOR_CONFIG.
setlocal
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p=$env:VERBEX_CONFIG; $d=Split-Path -Parent $p; if(-not (Test-Path $d)){ New-Item -ItemType Directory -Force -Path $d | Out-Null }; $raw=''; if(Test-Path $p){ $raw=Get-Content -Raw -Path $p }; if([string]::IsNullOrWhiteSpace($raw)){ $root=[PSCustomObject]@{} } else { $root=$raw | ConvertFrom-Json }; if($null -eq $root.mcpServers){ $root | Add-Member -NotePropertyName mcpServers -NotePropertyValue ([PSCustomObject]@{}) -Force }; $entry=[PSCustomObject]@{ url=$env:VERBEX_MCP_URL }; $root.mcpServers | Add-Member -NotePropertyName verbex -NotePropertyValue $entry -Force; [IO.File]::WriteAllText($p, ($root | ConvertTo-Json -Depth 20)); Write-Host ('Added verbex to ' + $p)"
