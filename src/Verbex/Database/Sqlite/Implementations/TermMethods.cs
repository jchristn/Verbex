...
UPDATE {prefix}_terms
SET document_frequency = MAX(document_frequency - COALESCE((SELECT document_frequency_delta FROM deltas WHERE deltas.term_id = {prefix}_terms.id), 0), 0),
    total_frequency = MAX(total_frequency - COALESCE((SELECT total_frequency_delta FROM deltas WHERE deltas.term_id = {prefix}_terms.id), 0), 0),
    last_update_utc = '{Sanitizer.FormatDateTime(now)}'
WHERE id IN (SELECT term_id FROM deltas);";
