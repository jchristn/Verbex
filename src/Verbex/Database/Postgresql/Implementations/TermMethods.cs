...
SET document_frequency = GREATEST(terms.document_frequency - deltas.document_frequency_delta, 0),
    total_frequency = GREATEST(terms.total_frequency - deltas.total_frequency_delta, 0),
    last_update_utc = '{Sanitizer.FormatDateTime(now)}'
FROM (
    SELECT term_id,
           COUNT(*)::int AS document_frequency_delta,
           COALESCE(SUM(term_frequency), 0)::int AS total_frequency_delta
    FROM {prefix}_document_terms
    WHERE document_id IN ({inClause})
    GROUP BY term_id
) AS deltas
WHERE terms.id = deltas.term_id;";
