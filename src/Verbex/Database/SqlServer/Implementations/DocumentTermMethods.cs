...
WHERE dt.term_id IN ({inClause});";
        ...
        public async Task<List<SearchMatch>> SearchAsync(string tablePrefix, IEnumerable<string> termIds, bool useAndLogic = false, int limit = 100, IEnumerable<string>? labels = null, IDictionary<string, string>? tags = null, CancellationToken token = default)
        {
            ...
            if (labelList != null && labelList.Count > 0)
            {
                ...
                {
                    labelConditions.Add($@"EXISTS (SELECT 1 FROM {prefix}_labels l WHERE l.document_id = d.id AND LOWER(l.label) = LOWER(N'{Sanitizer.Sanitize(label)}'))");
                }
            ...
            if (tags != null && tags.Count > 0)
            {
                ...
                {
                    tagConditions.Add($@"EXISTS (SELECT 1 FROM {prefix}_tags t WHERE t.document_id = d.id AND t.[key] = N'{Sanitizer.Sanitize(tag.Key)}' AND t.value = N'{Sanitizer.Sanitize(tag.Value)}')");
                }
