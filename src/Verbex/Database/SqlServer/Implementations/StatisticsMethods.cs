namespace Verbex.Database.SqlServer.Implementations
{
    ...
    internal class StatisticsMethods : IStatisticsMethods
    {
        ...
        public async Task<IndexStatistics> GetIndexStatisticsAsync(string tenantId, string indexId, CancellationToken token = default)
        {
            ...
            string docCountQuery = $"SELECT COUNT(*) FROM documents WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}';";
            ...
            string termCountQuery = $"SELECT COUNT(*) FROM terms WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}';";
            ...
            string totalTermsQuery = $"SELECT COALESCE(SUM(total_frequency), 0) FROM terms WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}';";
            ...
            string docLengthQuery = $"SELECT COALESCE(SUM(document_length), 0) FROM documents WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}';";
...
FROM terms t
WHERE t.tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND t.index_id = N'{Sanitizer.Sanitize(indexId)}' AND t.term = N'{Sanitizer.Sanitize(term)}';";
