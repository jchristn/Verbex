...
FROM (
    SELECT document_id, N'label' AS metadata_type, label AS metadata_key, CAST(NULL AS NVARCHAR(1024)) AS metadata_value
    FROM {prefix}_labels
    WHERE document_id IN ({inClause})
    UNION ALL
    SELECT document_id, N'tag' AS metadata_type, [key] AS metadata_key, value AS metadata_value
    FROM {prefix}_tags
    WHERE document_id IN ({inClause})
) AS metadata;";
