namespace Verbex.Database.SqlServer.Implementations
{
    ...
    /// <summary>
    /// SQL Server implementation of document methods.
    /// </summary>
    internal class DocumentMethods : IDocumentMethods
    {
        ...
        /// <inheritdoc />
        public async Task AddAsync(string tenantId, string indexId, string id, string name, string? contentSha256, int documentLength, CancellationToken token = default)
        {
            DateTime now = DateTime.UtcNow;
            string query = $@"
INSERT INTO documents (id, tenant_id, index_id, name, content_sha256, document_length, term_count, indexed_utc, last_update_utc, created_utc)
VALUES (
    N'{Sanitizer.Sanitize(id)}',
    N'{Sanitizer.Sanitize(tenantId)}',
    N'{Sanitizer.Sanitize(indexId)}',
    ...
    '{Sanitizer.FormatDateTime(now)}'
        ...
        {
            string query = $@"
SELECT id, tenant_id, index_id, name, content_sha256, document_length, term_count, indexed_utc, last_update_utc, created_utc
FROM documents
...
WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}' AND name = N'{Sanitizer.Sanitize(name)}';";
        ...
        public async Task<DocumentMetadata?> GetWithMetadataAsync(string tenantId, string indexId, string id, CancellationToken token = default)
        {
            ...
            string labelsQuery = $"SELECT label FROM labels WHERE document_id = N'{Sanitizer.Sanitize(id)}';";
            ...
            string tagsQuery = $"SELECT [key], value FROM tags WHERE document_id = N'{Sanitizer.Sanitize(id)}';";
...
WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}' AND id IN ({inClause});";
        ...
        {
            string query = $"SELECT COUNT(*) FROM documents WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}';";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        {
            string query = $"SELECT TOP 1 1 FROM documents WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}' AND id = N'{Sanitizer.Sanitize(id)}';";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        {
            string query = $"SELECT TOP 1 1 FROM documents WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}' AND name = N'{Sanitizer.Sanitize(name)}';";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
            return result.Rows.Count > 0;
        }

        /// <inheritdoc />
        public async Task UpdateAsync(string tenantId, string indexId, string id, string name, string? contentSha256, int documentLength, int termCount, CancellationToken token = default)
        {
            DateTime now = DateTime.UtcNow;
            string query = $@"
...
WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}' AND id = N'{Sanitizer.Sanitize(id)}';";
        ...
        {
            string countQuery = $"SELECT COUNT(*) FROM documents WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}' AND id = N'{Sanitizer.Sanitize(id)}';";
            DataTable countResult = await _Driver.ExecuteQueryAsync(countQuery, false, token).ConfigureAwait(false);
            bool exists = countResult.Rows.Count > 0 && Convert.ToInt64(countResult.Rows[0][0]) > 0;
            if (!exists) return false;

            string query = $"DELETE FROM documents WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}' AND id = N'{Sanitizer.Sanitize(id)}';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
            return true;
        }
        ...
        {
            string countQuery = $"SELECT COUNT(*) FROM documents WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}';";
            DataTable countResult = await _Driver.ExecuteQueryAsync(countQuery, false, token).ConfigureAwait(false);
            long count = countResult.Rows.Count > 0 ? Convert.ToInt64(countResult.Rows[0][0]) : 0;

            string query = $"DELETE FROM documents WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND index_id = N'{Sanitizer.Sanitize(indexId)}';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
            return count;
        }
