namespace Verbex.Database.SqlServer.Implementations
{
    ...
    internal class TagMethods : ITagMethods
    {
        ...
        public async Task SetAsync(string tenantId, string indexId, string id, string? documentId, string key, string? value, CancellationToken token = default)
        {
            ...
            if (documentId != null)
            {
                existsQuery = $"SELECT id FROM tags WHERE document_id = N'{Sanitizer.Sanitize(documentId)}' AND [key] = N'{Sanitizer.Sanitize(key)}';";
            }
            else
            {
                existsQuery = $"SELECT id FROM tags WHERE index_id = N'{Sanitizer.Sanitize(indexId)}' AND document_id IS NULL AND [key] = N'{Sanitizer.Sanitize(key)}';";
            }
            ...
            if (existsResult.Rows.Count > 0)
            {
                ...
                query = $"UPDATE tags SET value = {Sanitizer.FormatNullableString(value)}, last_update_utc = '{Sanitizer.FormatDateTime(now)}' WHERE id = N'{Sanitizer.Sanitize(existingId)}';";
            }
            else
            {
                query = $@"
INSERT INTO tags (id, document_id, index_id, [key], value, last_update_utc, created_utc)
VALUES (N'{Sanitizer.Sanitize(id)}', {Sanitizer.FormatNullableString(documentId)}, N'{Sanitizer.Sanitize(indexId)}', N'{Sanitizer.Sanitize(key)}', {Sanitizer.FormatNullableString(value)}, '{Sanitizer.FormatDateTime(now)}', '{Sanitizer.FormatDateTime(now)}');";
            }
        ...
        {
            string query = $"SELECT value FROM tags WHERE document_id = N'{Sanitizer.Sanitize(documentId)}' AND [key] = N'{Sanitizer.Sanitize(key)}';";
        ...
        {
            string query = $"SELECT [key], value FROM tags WHERE document_id = N'{Sanitizer.Sanitize(documentId)}';";
            DataTable dt = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        {
            string query = $"SELECT [key], value FROM tags WHERE index_id = N'{Sanitizer.Sanitize(indexId)}' AND document_id IS NULL;";
            DataTable dt = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
            Dictionary<string, string> result = new Dictionary<string, string>();
            foreach (DataRow row in dt.Rows)
            {
        ...
        {
            string query = $"SELECT DISTINCT [key] FROM tags WHERE index_id = N'{Sanitizer.Sanitize(indexId)}';";
            DataTable dt = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        {
            string query = $"SELECT DISTINCT document_id FROM tags WHERE index_id = N'{Sanitizer.Sanitize(indexId)}' AND [key] = N'{Sanitizer.Sanitize(key)}' AND document_id IS NOT NULL;";
            DataTable dt = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        {
            string query = $"SELECT DISTINCT document_id FROM tags WHERE index_id = N'{Sanitizer.Sanitize(indexId)}' AND [key] = N'{Sanitizer.Sanitize(key)}' AND value = N'{Sanitizer.Sanitize(value)}' AND document_id IS NOT NULL;";
            DataTable dt = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
            return dt.Rows.Cast<DataRow>().Select(r => r["document_id"]?.ToString() ?? string.Empty).ToList();
        }
        ...
        {
            string query = $"SELECT TOP 1 1 FROM tags WHERE document_id = N'{Sanitizer.Sanitize(documentId)}' AND [key] = N'{Sanitizer.Sanitize(key)}';";
            DataTable dt = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<bool> RemoveAsync(string tenantId, string indexId, string documentId, string key, CancellationToken token = default)
        {
            ...
            string query = $"DELETE FROM tags WHERE document_id = N'{Sanitizer.Sanitize(documentId)}' AND [key] = N'{Sanitizer.Sanitize(key)}';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
            return true;
        }
        ...
        {
            string checkQuery = $"SELECT TOP 1 1 FROM tags WHERE index_id = N'{Sanitizer.Sanitize(indexId)}' AND document_id IS NULL AND [key] = N'{Sanitizer.Sanitize(key)}';";
            DataTable dt = await _Driver.ExecuteQueryAsync(checkQuery, false, token).ConfigureAwait(false);
            if (dt.Rows.Count == 0) return false;

            string query = $"DELETE FROM tags WHERE index_id = N'{Sanitizer.Sanitize(indexId)}' AND document_id IS NULL AND [key] = N'{Sanitizer.Sanitize(key)}';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
            return true;
        }
        ...
        {
            string countQuery = $"SELECT COUNT(*) FROM tags WHERE document_id = N'{Sanitizer.Sanitize(documentId)}';";
            DataTable countResult = await _Driver.ExecuteQueryAsync(countQuery, false, token).ConfigureAwait(false);
            long count = countResult.Rows.Count > 0 ? Convert.ToInt64(countResult.Rows[0][0]) : 0;

            string query = $"DELETE FROM tags WHERE document_id = N'{Sanitizer.Sanitize(documentId)}';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
            return count;
        }
        ...
        {
            string countQuery = $"SELECT COUNT(*) FROM tags WHERE index_id = N'{Sanitizer.Sanitize(indexId)}';";
            DataTable countResult = await _Driver.ExecuteQueryAsync(countQuery, false, token).ConfigureAwait(false);
            long count = countResult.Rows.Count > 0 ? Convert.ToInt64(countResult.Rows[0][0]) : 0;

            string query = $"DELETE FROM tags WHERE index_id = N'{Sanitizer.Sanitize(indexId)}';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
            return count;
        }
    }
